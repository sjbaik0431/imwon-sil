...
(function () {
  ...
  /** LocalStorage 헬퍼 — 카피본 자동 격리 prefix 적용
   *  index.html에서 window.__IMWON_LS_PREFIX 를 'imwon.v01.' 식으로 설정하면
   *  modules.js 내부의 'imwon.userKpis' 같은 키도 자동으로 'imwon.v01.userKpis'로 변환됩니다.
   */
  function _imwonLsPrefix() {
    try { return window.__IMWON_LS_PREFIX || 'imwon.'; }
    catch (e) { return 'imwon.'; }
  }
  function _resolveKey(key) {
    if (typeof key !== 'string') return key;
    const pref = _imwonLsPrefix();
    if (pref === 'imwon.') return key;
    if (key.indexOf('imwon.') === 0) return pref + key.substring(6);
    return key;
  }
  const LS = {
    get: function (key, defaultVal) {
      try {
        const v = localStorage.getItem(_resolveKey(key));
    ...
    set: function (key, val) {
      try {
        localStorage.setItem(_resolveKey(key), JSON.stringify(val));
      } catch (_) { /* noop */ }
    }
  };
